...
describe("FileHandler", function() {
	...
	describe("_getConvertedFileAndCache", () =>
		...
		it("should _convertFile ", function(done){
			...
			this.PersistorManager.getFileStream = sinon.stub().callsArgWith(3, null, this.stubbedStream);
			...
			return this.handler._getConvertedFileAndCache(this.bucket, this.key, this.convetedKey, {}, (err, fsStream)=> {
				...
				this.PersistorManager.getFileStream.calledWith(this.bucket, this.convetedKey).should.equal(true);
				this.ImageOptimiser.compressPng.calledWith(this.stubbedPath).should.equal(true);
				fsStream.should.equal(this.stubbedStream);
				return done();
			});
		})
	);
