...
should = chai.should()
expect = chai.expect
modulePath = "../../../app/js/S3FilestoreManager.js"
SandboxedModule = require('sandboxed-module')

describe "S3FilestoreManagerTests", ->
	...
	describe "getFileStream", ->
		...
		it "should use correct key", (done)->
			...
			@S3FilestoreManager.getFileStream @bucketName, @key, @fsPath, (err)=>
	...
	describe "copyFile", ->
		...
		it "should use knox to copy file", (done)->
			...
			@S3FilestoreManager.copyFile @bucketName, @sourceKey, @destKey, (err)=>
				err.should.equal @error
	...
	describe "deleteDirectory", ->
		...
		it "should list the contents passing them onto multi delete", (done)->
			...
			@S3FilestoreManager.deleteDirectory @bucketName, @key, (err)=>
	...
	describe "deleteFile", ->
		...
		it "should use correct options", (done)->
			...
			@S3FilestoreManager.deleteFile @bucketName, @key, (err)=>
	...
	describe "checkIfFileExists", ->
		...
		it "should use correct options", (done)->
			...
			@S3FilestoreManager.checkIfFileExists @bucketName, @key, (err)=>
				opts = @request.args[0][0]
				assert.deepEqual(opts.aws, {key:@settings.s3.key, secret:@settings.s3.secret, bucket:@bucketName})
		...
		it "should return true for a 200", (done)->
			...
			@requires["request"] = @request
			@S3FilestoreManager = SandboxedModule.require modulePath, requires: @requires
			@S3FilestoreManager.checkIfFileExists @bucketName, @key, (err, exists)=>
